...
#define KEYNAME "SOFTWARE\\Datajake\\Timidity_VSTi"
#define PROJPAGE "https://github.com/datajake1999/Timidity_VSTi"
...
static BOOL RefreshDialog(HWND hWnd, Timidity* effect)
{
	if (hWnd && effect)
	{
		...
		ParamValue = effect->getParameter (kVoices)*MAX_VOICES;
		SendDlgItemMessage(hWnd, IDC_VOICES, TBM_SETPOS, TRUE, (LPARAM)ParamValue);
		effect->getParameterDisplay (kVoices, text);
		SetDlgItemText(hWnd, IDC_VOICEDISP, text);
		SendDlgItemMessage(hWnd, IDC_CTRL, TBM_SETRANGEMIN, 0, (VstInt32)effect->getSampleRate ()/MAX_CONTROL_RATIO);
		SendDlgItemMessage(hWnd, IDC_CTRL, TBM_SETRANGEMAX, 0, (VstInt32)effect->getSampleRate ());
		ParamValue = effect->getParameter (kControlRate)*effect->getSampleRate ();
		SendDlgItemMessage(hWnd, IDC_CTRL, TBM_SETPOS, TRUE, (LPARAM)ParamValue);
		effect->getParameterDisplay (kControlRate, text);
		SetDlgItemText(hWnd, IDC_CTRLDISP, text);
		...
		ParamValue = effect->getParameter (kImmediatePan);
		if (ParamValue >= 0.5)
		{
		...
		{
			CheckDlgButton(hWnd, IDC_PAN, BST_UNCHECKED);
		}
		ParamValue = effect->getParameter (kMono);
		if (ParamValue >= 0.5)
		{
		...
		{
			CheckDlgButton(hWnd, IDC_MONO, BST_UNCHECKED);
		}
		ParamValue = effect->getParameter (kFastDecay);
		if (ParamValue >= 0.5)
		{
			CheckDlgButton(hWnd, IDC_DECAY, BST_CHECKED);
		}
		else
		{
			CheckDlgButton(hWnd, IDC_DECAY, BST_UNCHECKED);
		}
		ParamValue = effect->getParameter (kAntialiasing);
		if (ParamValue >= 0.5)
		{
		...
		effect->getConfigName (text, MAX_PATH);
		SetDlgItemText(hWnd, IDC_CURCFG, text);
		return TRUE;
	}
	return FALSE;
}
...
static BOOL Scroll(HWND hWnd, LPARAM lParam, AudioEffectX* effect)
{
	if (hWnd && effect)
	{
		...
		LPARAM VoicesHandle = (LPARAM)GetDlgItem(hWnd, IDC_VOICES);
		LPARAM ControlHandle = (LPARAM)GetDlgItem(hWnd, IDC_CTRL);
		float ParamValue;
		...
		else if (lParam == VoicesHandle)
		{
			ParamValue = SendDlgItemMessage(hWnd, IDC_VOICES, TBM_GETPOS, 0, 0)/(float)MAX_VOICES;
			return SetParameterValue(effect, kVoices, ParamValue);
		}
		else if (lParam == ControlHandle)
		{
			ParamValue = SendDlgItemMessage(hWnd, IDC_CTRL, TBM_GETPOS, 0, 0)/effect->getSampleRate ();
			return SetParameterValue(effect, kControlRate, ParamValue);
		}
	}
	return FALSE;
}
...
static BOOL LoadConfiguration(HWND hWnd, Timidity* effect)
{
	if (hWnd && effect)
	{
		OPENFILENAME ofn;
		...
		if (RegOpenKeyEx(HKEY_CURRENT_USER, KEYNAME, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
		{
			...
			if (RegQueryValueEx(hKey, "DefaultDir", NULL, &type, (LPBYTE)directory, &len) == ERROR_SUCCESS)
			{
		...
		if (GetOpenFileName(&ofn))
		{
			...
			SetDlgItemText(hWnd, IDC_CURCFG, ofn.lpstrFileTitle);
			return TRUE;
		}
	}
	return FALSE;
...
static BOOL AboutBox(HWND hWnd)
{
	if (hWnd)
	{
		...
		LoadString((HINSTANCE)hInstance, IDS_ABOUTTXT, text, MAX_PATH*2);
		MessageBox(hWnd, text, caption, MB_ICONINFORMATION);
		return TRUE;
	}
	return FALSE;
...
static BOOL StatisticsBox(HWND hWnd, Timidity* effect)
{
	if (hWnd && effect)
	{
		char caption[MAX_PATH];
		...
		sprintf(text, temp, (VstInt32)effect->getSampleRate (), effect->getBlockSize (), g_useCount);
		MessageBox(hWnd, text, caption, MB_ICONINFORMATION);
		return TRUE;
	}
	return FALSE;
...
static BOOL HostInfoBox(HWND hWnd, Timidity* effect)
{
	if (hWnd && effect)
	{
		char caption[MAX_PATH];
...
static BOOL WINAPI DialogProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_PAN:
			...
			{
				return SetParameterValue(effect, kImmediatePan, 1);
			}
			else
			{
				return SetParameterValue(effect, kImmediatePan, 0);
			}
		case IDC_MONO:
			if (IsDlgButtonChecked(hWnd, IDC_MONO))
			{
				return SetParameterValue(effect, kMono, 1);
			}
			else
			{
				return SetParameterValue(effect, kMono, 0);
			}
		case IDC_DECAY:
			if (IsDlgButtonChecked(hWnd, IDC_DECAY))
			{
				return SetParameterValue(effect, kFastDecay, 1);
			}
			else
			{
				return SetParameterValue(effect, kFastDecay, 0);
			}
		case IDC_ANTI:
			if (IsDlgButtonChecked(hWnd, IDC_ANTI))
			{
				return SetParameterValue(effect, kAntialiasing, 1);
			}
			else
			{
		...
		case IDC_HIDEDISP:
			if (IsDlgButtonChecked(hWnd, IDC_HIDEDISP))
			{
				...
				ShowWindow(GetDlgItem(hWnd, IDC_VOICEDISP), SW_HIDE);
				ShowWindow(GetDlgItem(hWnd, IDC_CTRLDISP), SW_HIDE);
			}
			else
			{
				ShowWindow(GetDlgItem(hWnd, IDC_VOLDISP1), SW_SHOW);
...
bool Editor::open (void* ptr)
{
	...
	if (dlg)
	{
		...
		if (effect)
		{
			if (((Timidity*)effect)->getErrorText (text))
			{
				LoadString((HINSTANCE)hInstance, IDS_FAILCAP, caption, MAX_PATH);
...
void Editor::idle ()
{
	if (effect && dlg)
	{
		...
		double cpu = ((Timidity*)effect)->getCPULoad ();
		sprintf(text, "%lf %%", cpu);
